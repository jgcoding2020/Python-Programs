...
class PLC(object):  # process and state control (Process Life Cycle)
    ...
    def rdy(self, p):  # Scheduling
        self.multilevel_feedback_q(p)  # MLFQ
    ...
    def shortest_job_first(self, p):  # scheduling for SJF
        if self.p_running is None:

            self.evaluate_shortest_job()  # locates shortest job

            if p == self.shortest_p:  # when shortest process located
                self.rdy_q.remove(p)
                p.state = State.Running
                self.running(p)

    def evaluate_shortest_job(self):  # scheduling assist - SJF
        shortest = self.rdy_q[0]   # evaluate shortest job
        for i in range(1, len(self.rdy_q)):
            if self.rdy_q[i].burst < shortest.burst:
                shortest = self.rdy_q[i]
        self.shortest_p = shortest

    def round_robin(self, p, t_q):  # scheduling - Round Robin (RR)
        if self.p_running is None:  # similar to FCFS with tq
            p.counter = t_q
            self.rdy_q.remove(p)
            p.state = State.Running
            self.running(p)

    def multilevel_feedback_q(self, p):  # scheduling - MLFQ
        # when queue has higher priority or running processes are absent
        if self.p_running is None or p.q_number < self.p_running.q_number:

            q1_count = 0
            q2_count = 0
            q3_count = 0

            for element in self.rdy_q:  # queue count
                if element.q_number == 1:
                    self.q1.append(element)
                    q1_count += 1
                elif element.q_number == 2:
                    self.q2.append(element)
                    q2_count += 1
                elif element.q_number == 3:
                    self.q3.append(element)
                    q3_count += 1

            # display later
            self.info_string += "Queue:" + "\n" + "Q1: " + str(q1_count) + "\n" + "Q2: " + str(
                q2_count) + "\n" + "Q3: " + str(
                q3_count) + "\n"

            current_q = p.q_number

            # when a process has higher priority
            if self.p_running is not None and p.q_number < self.p_running.q_number:
                self.info_string += str(p.name) + " Preempted" + "\n"
                self.rdy_q.append(self.p_running)
                self.p_running.state = State.Ready
                self.p_running.counter = -1
                self.p_running.burst += 1
                self.p_running = None

            if q1_count > 0 and current_q == 1:  # depending on queues
                self.info_string += "Queue 1 " + str(p.name) + " in Queue " + str(current_q) + "\n"
                self.q1.remove(p)
                self.round_robin(p, 5)
            elif q2_count > 0 and current_q == 2 and q1_count == 0:
                self.info_string += "Queue 2 " + str(p.name) + " in Queue " + str(current_q) + "\n"
                self.q2.remove(p)
                self.round_robin(p, 10)
            elif q3_count > 0 and current_q == 3 and q1_count == 0 and q2_count == 0:
                self.info_string += "Queue 1 " + str(p.name) + " in Queue " + str(current_q) + "\n"
                self.q3.remove(p)
                self.first_come_first_serve(p)
...
def main():  # main function
    ...
    os = PLC([p1, p2, p3, p4, p5, p6, p7, p8], "MLFQ")
